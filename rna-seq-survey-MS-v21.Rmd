...
expr_gene_and_read_counts_to_plot <- counts_and_meta %>%
  ...
  pivot_longer (cols = starts_with("genes_expressed_above__"), names_to = "Expression_threshold", values_to = "Gene_count") %>%
  mutate(Read_type = factor(Read_type, levels = read_type_names)) %>%
  select(-starts_with("genes_expressed_above_"))
...
this_data <- this_data %>% 
  mutate(Expression_threshold = str_replace(Expression_threshold, "genes_expressed_above__", "genes > "))
  
  
these_stats <- this_data  %>%
group_by(Read_type, Expression_threshold) %>%
  summarize(r_corr = round(cor(Gene_count, Read_count, 
                               method = "spearman"), 2),
            r_p_value = cor.test(Gene_count, Read_count, 
...
```{r rna-seq-survey-MS-v20-47, fig.height=6, fig.width=6}
...
stats_display <- these_stats %>% 
  pivot_longer(c(r_corr, r_p_value)) %>%
  mutate(expr_col = paste0(Expression_threshold, "; ", str_replace(name, "r_p", "p"))) %>%
  select(-Expression_threshold, -name) %>%
  pivot_wider(names_from = "expr_col", 
              values_from = value) 
...
this_data  %>%
group_by(Read_type, Expression_threshold) %>%
  summarize(r_corr_pearson = round(cor(Gene_count, Read_count, method = "pearson"), 2),
            r_corr_spearman = round(cor(Gene_count, Read_count, method = "spearman"), 2),
            r_corr_kendall = round(cor(Gene_count, Read_count, method = "kendall"), 2)) %>%
  arrange(Expression_threshold)
...
# Figure 3
```{r rna-seq-survey-MS-v20-48, fig.height=8, fig.width=10}
