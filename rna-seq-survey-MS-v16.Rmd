...
#  select(c(dataset_id, project_id, project, starts_with("pct_"))) %>%
...
# 78 datasets in the project have fewer than 0.2 million MEND reads. 
...
ggplot(subset(counts_and_meta_long_one_proj, dataset_id %in% datasets_to_plot)) + 
  ...
  ggtitle(paste("Read counts for", plot_word, "datasets from", this_project)) +
  coord_flip()
...
ggplot(counts_and_meta %>% 
  ...
  ggtitle(paste("Relationship of dupe fraction to MEND count in", this_project), "For detecting whether datasets with less info were sequenced more deeply. \nNormally, higher depth datasets have more MEND reads and more duplicate \nreads than the same dataset sequenced at a lower total depth.")
