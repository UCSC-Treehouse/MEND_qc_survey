...
params:
  date: !r Sys.Date()
  this_dataset: "2019-2020_unofficial"
  this_analysis_style: "unweighted"
#  raw_read_survey_results_file: "read_counts_2020_05_26.txt"
---
...
V12 - exclude samples with fewer than 100 measured genes and use pre-generated survey_data
...
project_size_histogram <- ggplot(counts_and_meta) + 
  geom_histogram(aes(n_in_project, group=project), fill = "lightgrey", color = "black") +
  # theme_linedraw(base_size = 20) +
  theme_minimal(base_size = 20) +
  theme(axis.line.x = element_line(color="darkgrey", size = 0.5),
        axis.line.y = element_line(color="darkgrey", size = 0.5)) +  xlab("Project size") +
  ylab("Samples") + 
  theme(legend.position="none")

project_size_histogram

# for projects that are more than 75% one disease, color by that disease

n_distinct(counts_and_meta$project)
sort(unique(counts_and_meta$n_in_project))

counts_and_meta %>%
  select(project, n_in_project) %>%
  distinct %>%
  pull(n_in_project) %>%
  median
...
table(subset(counts_and_meta, is.na(pedaya))$sample_prefix)

n_ped_aya <- sum(counts_and_meta$pedaya=="Yes, age < 30 years" | grepl("TARGET", counts_and_meta$sample_prefix), na.rm = TRUE)
n_ped_aya
n_ped_aya/nrow(counts_and_meta)
# more than 95% of which are pediatric <30; of the samples with specific ages at diagnosis reported, the median was 7
...
IQR(counts_and_meta$seq_length, na.rm = TRUE)
 ...
 read_length_histogram <-  ggplot(counts_and_meta) + 
  geom_histogram(aes(seq_length, group=project), fill = "lightgrey", color = "black") +
#  theme_linedraw(base_size = 20) +
      theme_minimal(base_size = 20) +
    theme(axis.line.x = element_line(color="darkgrey", size = 0.5),
...
Total_read_counts <- counts_and_meta %>% 
  ungroup %>%
  mutate(sample_value = Total_reads/1e6) %>%
  summarize(variance= var(sample_value),
            min = min(sample_value),
            max = max(sample_value),
            median = median(sample_value),
            p25 = quantile(sample_value, 0.25),
            p75 = quantile(sample_value, 0.75),
            iqr = IQR(sample_value))
...
#  select(c(sample_id, project_id, project, starts_with("pct_"))) %>%
  pivot_longer(cols = starts_with("pct_"), names_to = "read_type_fraction_name", values_to = "sample_value") 
...
# in 77 samples, more than 25% of reads are unmapped
...
fiftypct <- read_counts_with_read_type_fractions %>%
  group_by(project) %>%
  summarize(has_a_sample_with_more_than_50pct_dupes = any(pct_dupe_of_mapped>0.50),
            median_pct_dupe_of_mapped = median(pct_dupe_of_mapped)) %>%
  mutate(median_lt_50 = median_pct_dupe_of_mapped < 0.5,
         median_lt_50_and_has_a_sample = median_lt_50 & has_a_sample_with_more_than_50pct_dupes)


fiftypct %>% tabyl(median_lt_50)
fiftypct %>% tabyl(has_a_sample_with_more_than_50pct_dupes)
fiftypct %>% tabyl(median_lt_50_and_has_a_sample)
```
...
fiftypcte <- read_counts_with_read_type_fractions %>%
  group_by(project) %>%
  summarize(has_a_sample_with_more_than_50pct_ne = any(pct_non_exonic_of_non_dupe>0.50),
            median_pct_dupe_of_mapped = median(pct_non_exonic_of_non_dupe)) %>%
  mutate(median_lt_50 = median_pct_dupe_of_mapped < 0.5,
         median_lt_50_and_has_a_sample = median_lt_50 & has_a_sample_with_more_than_50pct_ne)


fiftypcte %>% tabyl(median_lt_50)
fiftypcte %>% tabyl(has_a_sample_with_more_than_50pct_ne)
fiftypcte %>% tabyl(median_lt_50_and_has_a_sample)
```
...
comparison_of_remaining <- read_type_fractions_long %>% 
  ...
  group_by(read_type_fraction_name) %>%
  summarize(min = round(1-max(sample_value), 2),
    max = round(1-min(sample_value), 2),
    median = round(1-median(sample_value), 2),
    p25 = round(1-quantile(sample_value, 0.25), 2),
    p75 = round(1-quantile(sample_value, 0.75), 2)) %>%
...
plot_grid(project_size_histogram,
read_type_schematic,
read_length_histogram,
read_type_fractions_schematic,
                 nrow=2,
...
read_type_fractions_histogram <-  ggplot(read_type_fractions_long_for_histogram) +
  # geom_histogram(aes(x = sample_value, fill = read_type_fraction_name))  +
    geom_histogram(aes(x = sample_value, color = read_type_fraction_name), 
  ...
  ylab("Samples") +
  xlab(paste0("Read type percentages in ", length(unique(read_type_fractions_long_for_histogram$sample_id)),  " samples")) +
...
# 75% of  samples have fewer than 7% unmapped reads 
read_counts_with_read_type_fractions
...
MEND_pct <- read_counts_with_MEND_fractions %>% 
  ungroup %>%
  mutate(sample_value = pct_MEND_of_total) %>%
  summarize(variance= var(sample_value),
            min = min(sample_value),
            max = max(sample_value),
            median = median(sample_value),
...
# Show per-project distribution of read_type_fractions 
...
read_type_fractions_long_for_boxplot <- read_type_fractions_long_for_histogram %>%
  ungroup %>%
  mutate(boxplot_label = fct_reorder(paste0(as.character(project), ", n=", n_in_project), n_in_project))


read_type_fractions_per_project_boxplot <- ggplot(read_type_fractions_long_for_boxplot) +
  geom_boxplot(aes(x = sample_value, y=boxplot_label), outlier.size = 0.5)  +
  facet_wrap(~read_type_fraction_name, 
             nrow = 1, 
             labeller = labeller(
               read_type_fraction_name = stat_label
  ...
  ylab("Projects") +
  xlab(paste0("Read type percentages in ", length(unique(read_type_fractions_long_for_boxplot$sample_id)),  " samples")) +
  scale_x_continuous(breaks = seq(0, 1, by = 0.5)) +
    theme_minimal() +
    theme(axis.line.x = element_line(color="darkgrey", size = 0.5),
        axis.line.y = element_line(color="darkgrey", size = 0.5)) +
...
read_type_fractions_per_project_boxplot
...
```{r rna-seq-survey-MS-v16-34, fig.height=6, fig.width=8}
 ...
 counts_and_meta_long <- counts_and_meta  %>%
  pivot_longer(cols = c(Total_reads, Mapped, MEND, MND), names_to = "read_type_name", values_to = "sample_value")  %>%
   ungroup %>%
...
# 78 samples in the project have fewer than 0.2 million MEND reads. 
sum((counts_and_meta_long_one_proj %>% filter(read_type_name=="MEND"))$sample_value < 200000)
those_samples <- counts_and_meta_long_one_proj %>% filter(read_type_name=="MEND", sample_value < 200000) %>% pull(sample_id)


summary((counts_and_meta_long_one_proj %>% filter(read_type_name=="Total_reads", sample_id %in% those_samples))$sample_value)


max_plots_to_include <- 10
n_samples_to_plot <- ifelse(n_distinct(counts_and_meta_long_one_proj$sample_id)>max_plots_to_include, 
                         max_plots_to_include, 
                         n_distinct(counts_and_meta_long_one_proj$sample_id))
plot_word <- ifelse(n_samples_to_plot==max_plots_to_include, max_plots_to_include, paste("all", n_distinct(counts_and_meta_long_one_proj$sample_id)))

set.seed(100)
samples_to_plot <- sample(unique(counts_and_meta_long_one_proj$sample_id), n_samples_to_plot)

ggplot(subset(counts_and_meta_long_one_proj, sample_id %in% samples_to_plot)) + 
  geom_col(aes(x=read_type_name, y=sample_value/1e6, fill = read_type_name)) +
  ...
  ggtitle(paste("Read counts for", plot_word, "samples from", this_project)) +
  coord_flip()


ggplot(counts_and_meta_long_one_proj %>% filter(read_type_name == "MEND")) + 
  geom_histogram(aes(sample_value/1e6)) +
    theme_minimal(base_size = 20) +
    theme(axis.line.x = element_line(color="darkgrey", size = 0.5),
        axis.line.y = element_line(color="darkgrey", size = 0.5)) +
  ggtitle(paste("Distribution of MEND counts in", this_project))
...
ggplot(counts_and_meta %>% 
  ...
  ggtitle(paste("Relationship of dupe fraction to MEND count in", this_project), "For detecting whether samples with less info were sequenced more deeply. \nNormally, higher depth samples have more MEND reads and more duplicate \nreads than the same sample sequenced at a lower total depth.")

counts_and_meta_long_one_proj %>%
  select(seq_length, sample_id) %>%
  distinct %>%
  tabyl(seq_length)
...
counts_and_meta %>% 
  ...
  kable(caption = "Correlations with all samples", digits = 2)

```


## correlations with all but low gene count samples
...
counts_and_meta %>% 
  ...
  arrange(Read_type_count) %>%
  select(c(Read_type_count, starts_with("genes_expressed"))) %>%
  kable(caption = "Correlations with all but low gene count samples", digits = 2)
...
## Identify samples with unusual gene counts or read depths
```{r rna-seq-survey-MS-v16-39}

samples_with_normal_expressed_gene_numbers <- counts_and_meta %>%
  filter(genes_expressed_above_0 > min_genes_gt_0) %>%
  pull(sample_id)

samples_with_outlier_gene_counts <- counts_and_meta %>%
  ungroup %>%
  filter(is_outlier(genes_expressed_above_0)) %>%
  pull(sample_id) 

samples_with_outlier_depths <- counts_and_meta %>%
  ungroup %>%
...
plot_gene_count_correlations(expr_gene_and_read_counts_to_plot,
                             "Correlations calculated across all samples")

plot_gene_count_correlations(expr_gene_and_read_counts_to_plot %>% 
  filter(sample_id %in% samples_with_normal_expressed_gene_numbers),
  paste("Correlations in samples with more than", min_genes_gt_0, "genes expressed above 0"))

plot_gene_count_correlations(expr_gene_and_read_counts_to_plot %>% 
  filter(! sample_id %in% samples_with_outlier_gene_counts),
  paste("Correlations calculated across all but outlier gene count samples"))


plot_gene_count_correlations(expr_gene_and_read_counts_to_plot %>% 
  filter(! sample_id %in% samples_with_outlier_depths),
  paste("Correlations calculated across all but outlier read depth samples"))


plot_gene_count_correlations(expr_gene_and_read_counts_to_plot %>% 
  filter(! sample_id %in% samples_with_outlier_depths, 
         ! sample_id %in% samples_with_outlier_gene_counts
         ),
  paste("Correlations calculated across all but samples with outlier read depth or gene count"))


plot_gene_count_correlations(
  expr_gene_and_read_counts_to_plot %>% 
    filter(disease ==  "acute lymphoblastic leukemia"),
  "Correlations calculated among acute lymphoblastic leukemia samples")
...
this_data <- expr_gene_and_read_counts_to_plot %>% 
      filter(
       ...
       ! sample_id %in% exclude_for_depth, 
        ! sample_id %in% exclude_for_gene_count
      ) %>%
  mutate(Expression_threshold = str_replace(Expression_threshold, "genes_expressed_above_", "genes above "))

n_distinct(this_data$sample_id)

this_plot_title <- paste("Correlations calculated across all but samples with excessive read depth or low gene count")
...
cor_plots_excluding_high_read_depth_and_low_gene_count_samples <- ggplot(this_data,
       aes(x=Read_count/1E6, y=Gene_count/1e3, color = Read_type)) + 
  geom_point(alpha = 0.5, shape = 20, size =0.1) +
  geom_smooth(method = "lm", color = "black") +
  geom_label(data = these_stats, 
...
cor_plots_excluding_high_read_depth_and_low_gene_count_samples

```

# old
```{r rna-seq-survey-MS-v16-43}
# this_data <- expr_gene_and_read_counts_to_plot %>% 
#       filter(
#         Expression_threshold %in% paste0("genes_expressed_above_", 1:3),
#    #     ! sample_id %in% samples_with_outlier_depths, 
#    #     ! sample_id %in% samples_with_outlier_gene_counts
#       ) %>%
#   mutate(Expression_threshold = str_replace(Expression_threshold, "genes_expressed_above_", "genes above "))
# 
# this_plot_title <- paste("Correlations calculated across all but samples with outlier read depth or gene count")
# 
# these_stats <- this_data  %>%
# group_by(Read_type, Expression_threshold) %>%
#   summarize(r_corr = round(cor(Gene_count, Read_count), 2))
...
f2b <- read_type_fractions_per_project_boxplot +
            theme(strip.text.x = element_blank(),
                  strip.background = element_blank(),
                  strip.text = element_blank())
...
f2ab <- plot_grid(f2a, 
                  ...
                  align = "v",
                  labels = "AUTO")
f2c <- plot_grid(cor_plots_excluding_high_read_depth_and_low_gene_count_samples,
          labels = "C")
...
# Expressed genes and sub-exonic reads
## Load read_distribution results
```{r rna-seq-survey-MS-v16-45}
readDist_data_raw <- read_tsv("readDist/readDist_groomed.2020_06_09.txt")


readDist_data <- readDist_data_raw %>%
  rename(sample_id = source_file) %>%
...
expr_gene_and_sub_read_counts <- counts_and_meta %>%
    left_join(readDist_data, by=c("sample_id"))

```
# function for calculating correlations and plotting results

```{r rna-seq-survey-MS-v16-46}
...
## All samples
```{r rna-seq-survey-MS-v16-47, fig.height=8, fig.width=8}
plot_cors(expr_gene_and_sub_read_counts, "All samples")
```
## Subsets
```{r rna-seq-survey-MS-v16-48, fig.height=8, fig.width=8}

plot_cors(expr_gene_and_sub_read_counts%>% 
  filter(sample_id %in% samples_with_normal_expressed_gene_numbers),
          paste("samples with more than", min_genes_gt_0, "genes expressed above 0"))

plot_cors(expr_gene_and_sub_read_counts%>% 
  filter(! sample_id %in% samples_with_outlier_gene_counts),
          paste("samples with non-outlier gene counts"))

plot_cors(expr_gene_and_sub_read_counts%>% 
  filter(! sample_id %in% samples_with_outlier_depths, 
         ! sample_id %in% samples_with_outlier_gene_counts
         ),
          paste("samples with non-outlier gene counts and non outlier depths"))


plot_cors(expr_gene_and_sub_read_counts%>% 
  filter(disease ==  "acute lymphoblastic leukemia"),
          "All acute lymphoblastic leukemia samples")
```
